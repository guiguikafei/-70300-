...
function BrowsePanel:OnInitialize()
    ...
    local SearchInputItemLevel = GUI:GetClass('SearchBox'):New(self) do
        SearchInputItemLevel:SetSize(120, 15)
        SearchInputItemLevel:SetPoint('BOTTOMLEFT', MainPanel, 180, 5)
        SearchInputItemLevel:SetPrompt(L['装等过滤器'])
        SearchInputItemLevel:EnableAutoComplete(true)
        SearchInputItemLevel:EnableAutoCompleteFilter(false)
        SearchInputItemLevel:SetCallback('OnTextChanged', function()
    ...
    local ActivityList = GUI:GetClass('DataGridView'):New(self) do
	    ...
	    local leveltext = tonumber(SearchInputItemLevel:GetText())
	    
	    if leveltext == nil then
		leveltext = 0
            end	 	     	  
	   
            return activity:Match(...) and ((RAID_PROGRESSION_LIST[activity:GetActivityID()] and activity:GetItemLevel() >= leveltext and Profile:GetSetting('ItemLvEnabled') or not Profile:GetSetting('ItemLvEnabled')))
	    end)
...
function BrowsePanel:OnFilterButtonClicked()
        ...
        },
	{
            ...
            keepShownOnClick = true,
            checked = function()
                return Profile:GetSetting('ItemLvEnabled')
            end,
            func = function(_, _, _, checked)
                Profile:SetSetting('ItemLvEnabled', checked)
            end
        },
        {
            text = CANCEL
